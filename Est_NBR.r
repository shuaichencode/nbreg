...
# Version 1.1 (date: 9/8/2022)
# Author: Shuai Chen
# developed under R version 4.0.4

# The program allow factor covariates Z (but need to set as factor before using)
# Interactions between covariates and treatment in NBR must be a subset of main effects (included in Z)
# Will remove patients with missing values
...
#  if((is.null(Patition.times))&(is.null(Eff))) cat("Note: Patition.times is not provided. Assume all costs are evenly spreaded until follow-up time when truncated by time limit L.\n")
  ...
  cat("Censoring rate =",round(1-mean(deltaL),3)*100,"%\n")
  ...
  if(Method=="CC"){
    ...
    if(sum(1-deltaL)==0)cat("Method: Ordinary Least Squares\n")
    else {
  ...
  if(Method=="SW"){
	...
	if(sum(1-deltaL)==0)cat("Method: Doubly Robust Ordinary Least Squares (estimate is causal average INB)\n")
